...
PANEL_HEIGHT = 200  # Increased to accommodate additional info
...
def draw_panel(screen, bodies, trail_fade_speed, boundary_behavior, paused):
    ...
    trail_slider_y = HEIGHT + 140  # Increased to leave more space
    draw_slider(screen, 100, trail_slider_y, 600, 10, value, (150, 150, 150), "Trail Duration")

    # Draw the boundary behavior button
    button_x = WIDTH - 150  # Position it on the right side of the panel
    button_y = HEIGHT + 20
    button_width = 120
    button_height = 40
    ...
    pause_button_x = WIDTH - 150
    pause_button_y = HEIGHT + 80
...
def handle_slider_event(bodies, mouse_pos, trail_fade_speed):
    ...
    slider_x = 100
    slider_y = HEIGHT + 140  # Adjusted position
    slider_width = 600
    if slider_x <= mouse_pos[0] <= slider_x + slider_width and slider_y <= mouse_pos[1] <= slider_y + 10:
        relative_position = (mouse_pos[0] - slider_x) / slider_width
...
def main():
    ...
    boundary_behavior = "Bounce"  # Starting with bounce behavior
