...
RED = (255, 0, 0)            # Added RED color to fix NameError
...
# Color Gradient Function: Maps distance to color from red to blue
def distance_to_color(distance, max_distance):
    """
    Maps a distance value to a color from red (close) to blue (far).
    :param distance: The distance value of the node.
    :param max_distance: The maximum distance in the graph.
    :return: A tuple representing the RGB color.
    """
    if max_distance == 0:
        max_distance = 1  # Prevent division by zero
    ratio = min(distance / max_distance, 1.0)  # Normalize between 0 and 1

    # Define color gradient from red to yellow to green to cyan to blue
    if ratio < 0.2:
        # Red to Yellow
        r = 255
        g = int(255 * (ratio / 0.2))
        ...
        r = 0
        g = 255
        b = int(255 * ((ratio - 0.4) / 0.2))
    elif ratio < 0.8:
        # Cyan to Blue
        r = 0
        g = int(255 * (1 - (ratio - 0.6) / 0.2))
        b = 255
    else:
        # Blue to Dark Blue
        r = 0
        g = 0
        b = 255 - int(155 * ((ratio - 0.8) / 0.2))  # Fade to darker blue

    return (r, g, b)

# Draw the Graph
def draw_graph(screen, nodes, zoom, offset_x, offset_y, dragged_node, source_node):
    ...
    # Determine the actual maximum distance for color normalization
    actual_max_distance = 0
    for node in nodes:
        if node.distance < float('inf') and node.distance > actual_max_distance:
            actual_max_distance = node.distance
    ...
    for node in nodes:
        ...
        if 0 <= x <= WIDTH and 0 <= y <= (HEIGHT - PANEL_HEIGHT):
            if node == source_node:
                ...
                color = distance_to_color(node.distance, actual_max_distance)
            else:
                color = RED  # Default color for nodes not reached by traversal
...
# Draw the UI Panel
def draw_ui(screen, iterations_per_frame, zoom_level, num_nodes, edge_prob, energy, input_active, input_boxes, refresh_button_hover, drag_mode, drag_mode_hover):
    ...
    instructions = [
        ...
        "Nodes are colored from Red (unreachable) to Blue (far).",
...
def main():
    ...
    while running:
        ...
        # Determine the maximum distance for color normalization
        max_traversal_distance = 0
        for node in nodes:
            if node.distance < float('inf') and node.distance > max_traversal_distance:
                max_traversal_distance = node.distance

        # Clear the screen and draw the graph and UI elements
        draw_graph(screen, nodes, zoom, offset_x, offset_y, dragged_node, source_node)
        draw_ui(screen, iterations_per_frame, zoom_level, 
                ...
                energy,
                input_active, input_boxes, refresh_button_hover, drag_mode, drag_mode_button_hover)
        pygame.display.flip()
        clock.tick(60)
