...
MIN_SCALE = 1e8   # Original scale, most zoomed-in
MAX_SCALE = 1e10  # Maximum zoom-out level
LOG_MIN_SCALE = np.log10(MIN_SCALE)
LOG_MAX_SCALE = np.log10(MAX_SCALE)

# Define min and max dt for simulation accuracy
MIN_DT = 100      # Minimum time step size
MAX_DT = 10000    # Maximum time step size
LOG_MIN_DT = np.log10(MIN_DT)
LOG_MAX_DT = np.log10(MAX_DT)

# Define min and max simulation speed multipliers
MIN_SPEED_MULTIPLIER = 1     # Normal speed
MAX_SPEED_MULTIPLIER = 100   # 100 times faster
LOG_MIN_SPEED = np.log10(MIN_SPEED_MULTIPLIER)
LOG_MAX_SPEED = np.log10(MAX_SPEED_MULTIPLIER)

# Set up display
WIDTH, HEIGHT = 800, 600
PANEL_HEIGHT = 320  # Increased to accommodate additional sliders and buttons
screen = pygame.display.set_mode((WIDTH, HEIGHT + PANEL_HEIGHT))
pygame.display.set_caption('Three-Body Problem with Adjustable Time Step')
...
def draw_panel(screen, bodies, trail_fade_speed, paused, boundary_dropdown, input_boxes, scale, time_speed_multiplier, dt, pause_button_rect, reset_button_rect):
    ...
    # Adjusted positions for sliders
    slider_x = 100
    trail_slider_y = HEIGHT + 110  # Adjusted position
    zoom_slider_y = trail_slider_y + 40
    time_slider_y = zoom_slider_y + 40
    dt_slider_y = time_slider_y + 40
    ...
    draw_slider(screen, slider_x, trail_slider_y, trail_slider_width, 10, value, (150, 150, 150), "Trail Duration")
    ...
    zoom_value = max(0, min(zoom_value, 1))
    zoom_slider_width = 400
    draw_slider(screen, slider_x, zoom_slider_y, zoom_slider_width, 10, zoom_value, (150, 150, 150), "Zoom Level")

    # Time speed slider
    log_speed = np.log10(time_speed_multiplier)
    time_value = (log_speed - LOG_MIN_SPEED) / (LOG_MAX_SPEED - LOG_MIN_SPEED)
    time_value = max(0, min(time_value, 1))
    time_slider_width = 400
    draw_slider(screen, slider_x, time_slider_y, time_slider_width, 10, time_value, (150, 150, 150), "Time Speed")

    # dt slider
    log_dt = np.log10(dt)
    dt_value = (log_dt - LOG_MIN_DT) / (LOG_MAX_DT - LOG_MIN_DT)
    dt_value = max(0, min(dt_value, 1))
    dt_slider_width = 400
    draw_slider(screen, slider_x, dt_slider_y, dt_slider_width, 10, dt_value, (150, 150, 150), "Time Step Size")
    ...
    draw_button(screen, pause_button_rect.x, pause_button_rect.y, pause_button_rect.width, pause_button_rect.height, pause_text)

    # Draw the reset button
    draw_button(screen, reset_button_rect.x, reset_button_rect.y, reset_button_rect.width, reset_button_rect.height, "Reset")
...
def handle_slider_event(bodies, mouse_pos, trail_fade_speed, scale, time_speed_multiplier, dt):
    min_mass_exp = 25
    max_mass_exp = 32
    global LOG_MIN_SCALE, LOG_MAX_SCALE, LOG_MIN_SPEED, LOG_MAX_SPEED, LOG_MIN_DT, LOG_MAX_DT
    ...
    if slider_x <= mouse_pos[0] <= slider_x + slider_width and trail_slider_y <= mouse_pos[1] <= trail_slider_y + 10:
        relative_position = (mouse_pos[0] - slider_x) / slider_width
    ...
    # Zoom slider
    if slider_x <= mouse_pos[0] <= slider_x + slider_width and zoom_slider_y <= mouse_pos[1] <= zoom_slider_y + 10:
        relative_position = (mouse_pos[0] - slider_x) / slider_width
    ...
    # Time speed slider
    if slider_x <= mouse_pos[0] <= slider_x + slider_width and time_slider_y <= mouse_pos[1] <= time_slider_y + 10:
        relative_position = (mouse_pos[0] - slider_x) / slider_width
        relative_position = max(0, min(relative_position, 1))
        log_speed = LOG_MIN_SPEED + relative_position * (LOG_MAX_SPEED - LOG_MIN_SPEED)
        time_speed_multiplier = 10 ** log_speed
        slider_handled = True

    # dt slider
    if slider_x <= mouse_pos[0] <= slider_x + slider_width and dt_slider_y <= mouse_pos[1] <= dt_slider_y + 10:
        relative_position = (mouse_pos[0] - slider_x) / slider_width
        relative_position = max(0, min(relative_position, 1))
        log_dt = LOG_MIN_DT + relative_position * (LOG_MAX_DT - LOG_MIN_DT)
        dt = 10 ** log_dt
        slider_handled = True

    return trail_fade_speed, slider_handled, scale, time_speed_multiplier, dt

def handle_pause_button_event(mouse_pos, paused, pause_button_rect):
    if pause_button_rect.collidepoint(mouse_pos):
        paused = not paused
        return paused, True
    return paused, False

def handle_reset_button_event(mouse_pos, reset_button_rect):
...
def main():
    global scale
    bodies = init_bodies()
    dt = 1000  # Time step, will be adjusted by the slider
    offset_x = 0
    offset_y = 0
    is_panning = False
    ...
    # Initialize time speed multiplier
    time_speed_multiplier = 1  # Normal speed
    ...
    dropdown_x = slider_x + 420  # Place beside the sliders
    dropdown_y = trail_slider_y  # Align with the first slider vertically
    dropdown_width = 120
    dropdown_height = 30
    boundary_options = ["None", "Wrap", "Bounce"]
    boundary_dropdown = Dropdown(dropdown_x, dropdown_y, dropdown_width, dropdown_height,
                                 boundary_options, selected_index=boundary_options.index(boundary_behavior))

    # Positions for the pause button
    pause_button_x = dropdown_x
    pause_button_y = dropdown_y + dropdown_height + 10
    pause_button_width = dropdown_width
    pause_button_height = 30
    pause_button_rect = pygame.Rect(pause_button_x, pause_button_y, pause_button_width, pause_button_height)

    # Positions for the reset button
    reset_button_x = pause_button_x
    reset_button_y = pause_button_y + pause_button_height + 10  # Slightly below the pause button
    reset_button_width = pause_button_width
    reset_button_height = pause_button_height
    ...
    input_boxes = []
    for i, body in enumerate(bodies):
        slider_x_body = 50 + i * 250
        slider_y_body = HEIGHT + 20
        line_height = 18  # or use font.get_linesize()
        font_size = 18

        # Position InputBox
        pos_input_box = InputBox(
            slider_x_body, slider_y_body + 15 + line_height, 200, line_height + 10,
            text=f"{body.position[0]:.2e}, {body.position[1]:.2e}",
            font_size=font_size
        )
        # Velocity InputBox
        vel_input_box = InputBox(
            slider_x_body, slider_y_body + 15 + 2 * line_height, 200, line_height + 10,
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not over_body:
                    if mouse_pos[1] > HEIGHT:
                        # Update handle_slider_event to return dt
                        trail_fade_speed, slider_handled, scale, time_speed_multiplier, dt = handle_slider_event(
                            bodies, mouse_pos, trail_fade_speed, scale, time_speed_multiplier, dt)
                        paused, pause_button_handled = handle_pause_button_event(
                            mouse_pos, paused, pause_button_rect)
                        reset_button_handled = handle_reset_button_event(mouse_pos, reset_button_rect)
                        ...
                        if not slider_handled and not pause_button_handled and not reset_button_handled and boundary_selection is None:
                            # Start panning if no slider or button was adjusted
                            is_panning = True
                            pan_start_mouse_pos = mouse_pos
                            pan_start_offset = (offset_x, offset_y)
                        if reset_button_handled:
                            # Reset the simulation
                            bodies = init_bodies()
                            offset_x = 0
                            offset_y = 0
                            scale = 1e8  # Reset to initial scale
                            paused = False  # Or keep the same?
                            trail_fade_speed = 1e10  # Reset trail
                            time_speed_multiplier = 1  # Reset time speed multiplier
                            dt = 1000  # Reset dt
                            # Clear the trail_surface
                            trail_surface.fill((0, 0, 0, 0))

                            # Re-initialize InputBoxes
                            input_boxes = []
                            for i, body in enumerate(bodies):
                                slider_x_body = 50 + i * 250
                                slider_y_body = HEIGHT + 20
        ...
        if not paused:
            ...
            # Update positions and velocities multiple times per frame
            steps_per_frame = int(time_speed_multiplier)
                ...
                for body in bodies:
                    if trail_fade_speed is not None:
                        ...
                        if trail_fade_speed > 0:
                            for trail_point in body.positions:
                                trail_point['alpha'] -= trail_fade_speed / steps_per_frame
        ...
        draw_panel(screen, bodies, trail_fade_speed, paused, boundary_dropdown, input_boxes,
                   scale, time_speed_multiplier, dt, pause_button_rect, reset_button_rect)
