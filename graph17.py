...
PURPLE = (128, 0, 128)      # Distinct color for the source node (will change to BLACK)
...
# Function to map probability to color normalized about the mean
def probability_to_color(prob, mean_prob, min_prob, max_prob):
    """
    Maps a probability value to a color gradient normalized around the mean probability.
    - Probabilities below the mean transition from Blue to Purple.
    - Probabilities above the mean transition from Purple to Red.
    - Probabilities equal to the mean are Purple.
    """
    if prob > mean_prob:
        if max_prob == mean_prob:
            normalized = 0.0
        else:
            normalized = (prob - mean_prob) / (max_prob - mean_prob)
        normalized = max(0.0, min(1.0, normalized))
        red = 128 + int(normalized * (255 - 128))        # From 128 to 255
        green = 0
        blue = 128 - int(normalized * 128)              # From 128 to 0
    elif prob < mean_prob:
        if mean_prob == min_prob:
            normalized = 0.0
        else:
            normalized = (prob - min_prob) / (mean_prob - min_prob)
        normalized = max(0.0, min(1.0, normalized))
        red = int(normalized * 128)                      # From 0 to 128
        green = 0
        blue = 255 - int(normalized * 127)              # From 255 to 128
    else:
        red, green, blue = (128, 0, 128)                 # Purple
    return (red, green, blue)

# Draw the Graph
def draw_graph(screen, nodes, adj_matrix, zoom, offset_x, offset_y, dragged_node, source_node, wave_step, wave_vector, mean_prob, min_prob, max_prob):
    ...
    for idx, node in enumerate(nodes):
        ...
        if 0 <= x <= WIDTH and 0 <= y <= (HEIGHT - PANEL_HEIGHT):
            if node == source_node:
                color = BLACK  # Changed from PURPLE to BLACK for the source node
            elif wave_vector is not None and wave_vector[idx] > 0:
                # Color based on normalized probability around the mean
                color = probability_to_color(wave_vector[idx], mean_prob, min_prob, max_prob)
            else:
            ...
            # Draw a distinct outline around the source node for better visibility
            if node == source_node:
...
def draw_legend(screen):
    ...
    The gradient transitions from Blue (Low) -> Purple (Mean) -> Red (High).
    """
    legend_width = 300
    legend_height = 20
    legend_x = 50
    ...
    for x in range(legend_width):
        if x < legend_width / 2:
            # Blue to Purple
            normalized = x / (legend_width / 2)
            red = int(0 + normalized * (128 - 0))          # From 0 to 128
            green = 0
            blue = int(255 - normalized * (255 - 128))    # From 255 to 128
        else:
            # Purple to Red
            normalized = (x - legend_width / 2) / (legend_width / 2)
            red = int(128 + normalized * (255 - 128))      # From 128 to 255
            green = 0
            blue = int(128 - normalized * 128)            # From 128 to 0
    ...
    # Add labels
    min_label = FONT.render("Low", True, BLACK)
    middle_label = FONT.render("Mean", True, BLACK)
    max_label = FONT.render("High", True, BLACK)
    screen.blit(min_label, (legend_x, legend_y + legend_height + 5))
    screen.blit(middle_label, (legend_x + legend_width//2 - middle_label.get_width()//2, legend_y + legend_height + 5))
...
def main():
    ...
    while running:
                    ...
                    if drag_mode:
                        ...
                        for idx, node in enumerate(nodes):
                            ...
                            if distance_sq <= (node_radius + 5) ** 2:  # 5 pixels margin
                                ...
                                for n in nodes:
                                    n.color = GRAY
                                # Highlight the source node by setting it to BLACK
        ...
        if wave_step > 0 and source_node is not None:
            ...
            # Compute mean and min/max probabilities
            mean_prob = wave_vector.mean()
            min_prob = wave_vector.min()
            max_prob = wave_vector.max()
            # Update node colors based on wave_vector and mean_prob
            for idx, node in enumerate(nodes):
                if wave_vector[idx] > 0:
                    # Set color based on normalized probability around the mean
                    color = probability_to_color(wave_vector[idx], mean_prob, min_prob, max_prob)
                    node.color = color
                else:
                    if node != source_node:
                        node.color = GRAY
            # Ensure the source node is highlighted as BLACK
            source_node.color = BLACK
        else:
            wave_vector = None
            mean_prob = 0.5  # Default mean for the legend
            min_prob = 0.0
            max_prob = 1.0
        ...
        draw_graph(screen, nodes, adj_matrix, zoom, offset_x, offset_y, dragged_node, source_node, wave_step, wave_vector, mean_prob, min_prob, max_prob)
