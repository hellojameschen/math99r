...
GREEN = (0, 255, 0)
YELLOW = (255, 255, 0)      # Color for highlighted nodes and edges
...
RED = (255, 0, 0)            # Color for unreachable nodes
...
def apply_forces(nodes):
    ...
    for node in nodes:
        ...
        for other_node in node.edges:
            edge = tuple(sorted((id(node), id(other_node))))
...
# Draw the Graph
def draw_graph(screen, nodes, zoom, offset_x, offset_y, dragged_node, source_node, highlight_distance):
    ...
    for node in nodes:
        ...
        if 0 <= x <= WIDTH and 0 <= y <= (HEIGHT - PANEL_HEIGHT):
            if node == source_node:
                ...
                color = GRAY  # Reachable but not highlighted
            else:
...
def draw_ui(screen, iterations_per_frame, zoom_level, num_nodes, edge_prob, energy, input_active, input_boxes, 
    ...
    instructions = [
        ...
        "Nodes at the selected distance are highlighted in Yellow.",
...
def main():
    ...
    while running:
                    ...
                    if not neighbor.visited:
                        ...
                        if new_distance < neighbor.distance:
                            ...
                            if not neighbor.in_queue:
                                heapq.heappush(traversal_queue, (neighbor.distance, nodes.index(neighbor)))
        ...
        draw_graph(screen, nodes, zoom, offset_x, offset_y, dragged_node, source_node, highlight_distance)
