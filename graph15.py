    ...
    for idx, node in enumerate(nodes):
        ...
        if 0 <= x <= WIDTH and 0 <= y <= (HEIGHT - PANEL_HEIGHT):
            if node == source_node:
                ...
                # Color intensity based on wave probability
                intensity = min(255, int(wave_vector[idx] * 255))
                color = (255 - intensity, 0, intensity)  # Gradient from Red to Blue
            else:
        ...
        if wave_step > 0 and source_node is not None:
            ...
            for idx, node in enumerate(nodes):
                if wave_vector[idx] > 0:
                    # Example: Set color intensity based on wave_vector value
                    intensity = min(255, int(wave_vector[idx] * 255))
                    node.color = (255 - intensity, 0, intensity)  # Gradient from Red to Blue
                else:
                    if node != source_node:
                        node.color = GRAY
