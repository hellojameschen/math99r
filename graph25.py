...
def generate_refined_graph(nodes, adj_matrix, triangles, num_refinements=1):
    """
    Performs barycentric refinement on the graph by subdividing existing triangles.

    :param nodes: List of Node objects.
    :param adj_matrix: Adjacency matrix representing the graph.
    :param triangles: List of triangles (each triangle is a tuple of node indices).
    :param num_refinements: Number of times to perform the refinement.
    :return: Updated nodes, adj_matrix, and triangles after refinement.
    """
...
# Draw UI Panel
def draw_ui(screen, sliders, buttons, energy, shrink_input_text):
...
def main():
    ...
    while running:
        for event in pygame.event.get():
            ...
            if refine_button.handle_event(event):
                if len(triangles) == 0:
                    ...
                    nodes, adj_matrix, triangles = generate_refined_graph(nodes, adj_matrix, triangles, num_refinements=1)
        ...
        # Draw UI Elements
        draw_ui(screen, [slider], buttons, energy, shrink_input_text)
