...
# *** Change: Define min and max simulation speed multipliers
...
def draw_panel(screen, bodies, trail_fade_speed, paused, boundary_dropdown, input_boxes, scale, time_speed_multiplier, pause_button_rect, reset_button_rect):
    ...
    # Adjust zoom_value to prevent exceeding the original scale
    ...
    time_slider_y = zoom_slider_y + 50  # Position below zoom slider
    time_slider_width = 400
    draw_slider(screen, 100, time_slider_y, time_slider_width, 10, time_value, (150, 150, 150), "Time Speed")

    # Draw the boundary behavior dropdown
    boundary_dropdown.draw(screen)

    # Draw the pause button
    pause_button_x = pause_button_rect.x
    pause_button_y = pause_button_rect.y
    pause_button_width = pause_button_rect.width
    pause_button_height = pause_button_rect.height
    ...
    reset_button_x = reset_button_rect.x
    reset_button_y = reset_button_rect.y
    reset_button_width = reset_button_rect.width
    reset_button_height = reset_button_rect.height
    draw_button(screen, reset_button_x, reset_button_y, reset_button_width, reset_button_height, "Reset")
...
def handle_slider_event(bodies, mouse_pos, trail_fade_speed, scale, time_speed_multiplier):
    min_mass_exp = 25
    max_mass_exp = 32
    global LOG_MIN_SCALE, LOG_MAX_SCALE, LOG_MIN_SPEED, LOG_MAX_SPEED
    ...
    if slider_x <= mouse_pos[0] <= slider_x + slider_width and slider_y <= mouse_pos[1] <= slider_y + 10:
        ...
        # Adjust log_scale calculation to prevent zooming in beyond original scale
        log_scale = LOG_MIN_SCALE + relative_position * (LOG_MAX_SCALE - LOG_MIN_SCALE)
        scale = 10 ** log_scale
        slider_handled = True

    # *** Change: Handle time speed slider
    time_slider_y = zoom_slider_y + 50  # Should match the y-position in draw_panel
    slider_y = time_slider_y
    if slider_x <= mouse_pos[0] <= slider_x + slider_width and slider_y <= mouse_pos[1] <= slider_y + 10:
        relative_position = (mouse_pos[0] - slider_x) / slider_width
        relative_position = max(0, min(relative_position, 1))
    ...
    return trail_fade_speed, slider_handled, scale, time_speed_multiplier

def handle_pause_button_event(mouse_pos, paused, boundary_dropdown, pause_button_rect):
...
def main():
    ...
    # *** Change: Initialize time speed multiplier
    ...
    # Create a pygame.Rect for the pause button
    ...
    # Create a pygame.Rect for the reset button
    ...
    while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if not over_body:
                    if mouse_pos[1] > HEIGHT:
                        # Update handle_slider_event to return time_speed_multiplier
                        trail_fade_speed, slider_handled, scale, time_speed_multiplier = handle_slider_event(bodies, mouse_pos, trail_fade_speed, scale, time_speed_multiplier)
                        paused, pause_button_handled = handle_pause_button_event(mouse_pos, paused, boundary_dropdown, pause_button_rect)
        ...
        if not paused:
            ...
            # *** Change: Update positions and velocities multiple times per frame
        ...
        # Pass time_speed_multiplier to draw_panel
        draw_panel(screen, bodies, trail_fade_speed, paused, boundary_dropdown, input_boxes, scale, time_speed_multiplier, pause_button_rect, reset_button_rect)
